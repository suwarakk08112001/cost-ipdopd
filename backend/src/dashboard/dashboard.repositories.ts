...
export class DashboardRepositories {
  ...
  async findDep(dto: SearchDashboardDto) {
    ...
    const timeEnd =
      dto.timeEnd ?? (isToday ? dayjs().format('HH:mm:ss') : '23:59:59');

    const result = await this.prisma.$queryRaw<ResponsePttypeORDep[]>`
      WITH base AS (
        SELECT
    ...
    // แปลง BigInt เป็น Number ก่อนส่งกลับ ป้องกัน JSON.stringify error
  ...
  async findPttype(dto: SearchDashboardDto) {
    ...
    const timeStart = dto.timeStart ?? '00:00:00';
    const isToday = dayjs(datePicker).isSame(dayjs(), 'day');
    // const timeEnd = dto.timeEnd ?? dayjs().format('HH:mm:ss');
    const timeEnd =
      dto.timeEnd ?? (isToday ? dayjs().format('HH:mm:ss') : '23:59:59');
    console.log(timeEnd);

    const pttype = await this.prisma.$queryRaw<ResponsePttypeORDep[]>`
    WITH ovst_stat AS (
      SELECT
        o.pttype,
        p.name,
  ...
  async findIpdWard(dto: SearchDashboardDto) {
    const datePicker = dto.datePicker || new Date().toISOString().slice(0, 10); // 'YYYY-MM-DD'
    const ipdWard = await this.prisma.$queryRaw<ResponseIpdPttypeORIpdWard[]>`
      WITH ward_stat AS (
        SELECT
          i.ward,
          w.name AS ward_name,
  ...
  async findIpdPttype(dto: SearchDashboardDto) {
    // const datePicker = dto.datePicker;
    const datePicker = dto.datePicker || new Date().toISOString().slice(0, 10); // 'YYYY-MM-DD'
    console.log(datePicker);
    const IpdPttype = await this.prisma.$queryRaw<ResponseIpdPttypeORIpdWard[]>`
    WITH dept_stat AS (
        SELECT
          i.pttype,
  ...
  async findIpdBed() {
    const IpdBed = await this.prisma.$queryRaw<ResponseIpdBed[]>`
      WITH admit AS (
        SELECT 
          ward, 
